...
from sklearn.metrics import classification_report, roc_auc_score, average_precision_score, f1_score, confusion_matrix, precision_recall_curve
...
def evaluate_model(model, X_test, y_test, dataset_name, model_type, config):
    ...
    metrics['total_cost'] = metrics['false_positive_cost'] + metrics['false_negative_cost']
...
def generate_feature_importance(model, X_test, dataset_name, model_type):
    ...
    try:
        ...
        logger.error(f"Feature importance failed for {dataset_name} {model_type}: {str(e)}")
        return None

def train_and_evaluate(dataset_name, model_type, train_path, test_path, target_col, config):
    ...
    try:
        ...
        # Evaluate model
        metrics = evaluate_model(model, X_test, y_test, dataset_name, model_type, config)
...
if __name__ == "__main__":
    ...
    logger.info("\nSummary of results:")
    logger.info(results_df[['dataset', 'model', 'auc_roc', 'auc_pr', 'f1_score', 'min_cost']].to_string())