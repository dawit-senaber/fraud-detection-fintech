...
def validate_data(df):
    ...
    if not df["Class"].isin([0, 1]).all():
        ...
        raise ValueError(
            f"Target column 'Class' contains invalid values: {invalid_classes}"
        )
...
def create_preprocessing_pipeline():
    ...
    preprocessor = ColumnTransformer(
        transformers=[
            ...
            (
                "log_amt",
                Pipeline([("log", log_transformer), ("scale", log_scaler)]),
                ["Amount"],
            ),
        ],
...
def preprocess_creditcard(input_path, output_path, config):
    ...
    try:
        ...
        cc_data["Transaction_Value_Ratio"] = (
            cc_data["Amount"] / cc_data["Amount"].mean()
        )
        ...
        for name, trans, cols in preprocessor.transformers_:
            if name == "log_amt":
                ...
                remainder_cols = features.columns.difference(
                    ["Time", "Amount", "Class"]
                )
        ...
        joblib.dump(
            preprocessor,
            os.path.join(
                config.get("models_dir", "./models"), "creditcard_preprocessor.pkl"
            ),
        )
...
if __name__ == "__main__":
    config = load_config()
    input_path = config.get("data", {}).get(
        "creditcard_raw", "./data/raw/creditcard.csv"
    )
    output_path = config.get("data", {}).get(
        "processed_creditcard", "./data/processed/processed_creditcard.csv"
    )
